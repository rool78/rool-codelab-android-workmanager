...
class SelectImageActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            when (requestCode) {
                ...
                else -> println("Unknown request code.")
            }
        } else {
            println(String.format("Unexpected Result code %s", resultCode))
        }
    }
    ...
    private fun handleImageRequestResult(intent: Intent) {
        ...
        if (imageUri == null) {
            println("Invalid input image Uri.")
            return
        }
        println("???? handleImageRequestResult->${imageUri}")
